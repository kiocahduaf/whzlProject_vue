<template>
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="100px">
      <el-form-item label="用户名称" prop="companyname">
        <el-input v-model="dataForm.companyname" placeholder="用户名称, 如: 张安"></el-input>
      </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/company/user/info/${this.dataForm.id}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/company/user/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
